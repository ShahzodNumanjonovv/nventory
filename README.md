...
The suite covers FIFO allocation, oversell, partial refunds on both sides,
provider-mismatch rejection, historical storage snapshots, and profit
calculation. Tests run against in-memory SQLite (`aiosqlite`) for speed —
race-condition coverage (`SELECT FOR UPDATE` semantics) belongs in a separate
Postgres integration suite.

## Concurrency

`create_order`, `refund_to_provider`, and `refund_from_client` all acquire
`SELECT ... FOR UPDATE` row locks on the rows they intend to mutate before
computing availability. Concurrent transactions that touch the same stock
serialize at the lock and the second one re-reads the (now-updated)
availability, preventing overselling.

## Security

The API ships **without authentication**. It is intended to run on an internal
network or behind an authenticating reverse proxy / VPN / mTLS — not exposed
directly to the public internet. If you need user-level access control, add an
auth dependency (e.g. JWT or session) to the routers and require it in `deps.py`.
